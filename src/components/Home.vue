...
<template>
  <v-container max-width="1200" fluid>
    ...
    <v-img class="mb-1" height="60" src="@/assets/sisloc.png" />
    <div class="mb-8 text-center">
      <h1 class="text-h4 font-weight-bold ml-16 text-orange-darken-3">LocBox</h1>
    </div>

    <v-row v-if="currentFolder == 'root'">
      <v-col cols="12">
    ...
    <v-row>
      <v-col cols="12" v-if="dataTable.length > 0">
        <v-data-table class="mb-8" rounded="lg" :items="dataTable" :headers=headers :items-per-page="itemsPerPage" :page.sync="page">
          <template v-slot:item="{ item }: { item: any }">
            <tr class="text-wrap cursor-pointer" @click="openItem(item.type, item.name)">
  ...
  <new-folder-dialog @response="fetchFolders" v-model="showCreateFolderDialog" title="New folder"
